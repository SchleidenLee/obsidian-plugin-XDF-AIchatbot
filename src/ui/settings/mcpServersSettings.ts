...
export function displayMcpServersSettings(containerEl: HTMLElement, ctx: SettingsContext): void {
  ...
  new Setting(containerEl).setName("XDF Toolkits").setHeading();
  new Setting(containerEl)
    .setName("自动连接 xdf-toolkits")
    .setDesc("从同库 XDF Toolkits 插件读取本机 MCP 地址和令牌，写入下方服务器列表。")
    .addToggle((t) =>
      t.setValue(plugin.settings.xdfAutoBindToolkits !== false).onChange(async (v) => {
        plugin.settings.xdfAutoBindToolkits = v;
